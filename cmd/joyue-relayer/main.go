...
func (r *Relayer) sendTransaction(ctx context.Context, event *CrossShardRequestEvent) {
	...
	log.Printf("[INFO] Sent transaction successfully (txHash: %s, requestId: %d, targetShard: %d, target: %s)",
		signedTx.Hash().Hex(), event.RequestID, event.TargetShardID, event.Target.Hex())
}
